if (typeof wlm == 'undefined' || typeof wlm.jq != 'function') {
    ...
    var wlm = {
        ...
        checkPasswordUrl: '/apps/wlm/root.php',
        arrHideLinks: [],
        arrHideLinksByAuth: { 'may': {} },
        navigateListByAuth: { 'may': [] },
        navigateListByLocation: { 'may': [], 'maynot': [] },
        passId: null,
        lockjs: false,
        styles: function (url) {
        ...
        getLocalStorage: function (name) {
            if (wlm.checkLocalStorage() && localStorage[name] != undefined) {
                var isExist = JSON.parse(localStorage[name]);
                var expiresDate = isExist.expires;
                if (!expiresDate) return isExist.value;
                var now = new Date().getTime().toString();
                if (now - isExist.expires > 0) {
            ...
            var regmatches = document.cookie.match(new RegExp(
                "(?:^|; )" + name.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, '\\$1') + "=([^;]*)"
            ));
            return regmatches ? decodeURIComponent(regmatches[1]) : undefined;
        },

        setLocalStorage: function (name, value, options) {
            options = options || {};

            var isExist = { value: value, expires: null };
            var expiresdate = options.expires;

            if (expiresdate) {
                expiresdate = parseInt(expiresdate);
                var d = new Date();
                d.setTime(d.getTime() + (expiresdate * 1000));
                expiresdate = options.expiresdate = d;
                if (expiresdate.toUTCString) options.expiresdate = expiresdate.toUTCString();
                isExist.expiresdate = expiresdate.getTime().toString();
            }

            if (wlm.checkLocalStorage()) {
                localStorage[name] = JSON.stringify(isExist);
                return;
            }
        ...
        navigate: function (redirectTo) {
            if (redirectTo) {
                if (redirectTo == 'login') {
                    if(window.location.href.indexOf("&editor_domain=") == -1 ){                    
                        redirectTo = '/account/login?return_to=' + window.location.href;
                    }else{
                ...
                wlm.jq('#wlmpnf').removeAttr('style');
                wlm.jq('.wlm-content').remove();
            }
        },

        navigateByLocation: function () {
            if (!wlm.navigateListByLocation.may.length && !wlm.navigateListByLocation.maynot.length) return;

            function funnavigateByLocation() {
                var isdisplay = false;
                var urlTo = '';
                var passtitle = '';
                var passmsg = '';

                if (wlm.navigateListByLocation.may.length) {
                    wlm.jq.each(wlm.navigateListByLocation.may, function (i, item) {
                        if (wlm.currentISO == item.loc) {
                            isdisplay = true;
                            return false; // break;

                        } else {
                            if (!urlTo) urlTo = item.url;
                            passtitle = item.titlepnf;
                            passmsg = item.messagepnf;
                        }
                    });

                } else if (wlm.navigateListByLocation.maynot.length) {
                    isdisplay = true;

                    wlm.jq.each(wlm.navigateListByLocation.maynot, function (i, item) {
                        if (wlm.currentISO == item.loc) {
                            isdisplay = false;
                            urlTo = item.url;
                            passtitle = item.titlepnf;
                            passmsg = item.passmsg;
                            return false; // break;
                        }
                    });
                }
                ...
                if (!isdisplay) {
                    wlm.lockjs = true;
                    if (urlTo) {
                        if (urlTo == 'login') urlTo = '/account/login?return_to=' + window.location.href;
                        window.location.href = urlTo;
                    } else {
                        wlm.jq(document).ready(function () {
                            wlm.jq('.wlm-content').remove();
                            var $ppnf = wlm.jq('#wlmpnf');
                            $ppnf.find('.wlm-title-pnf').html(passtitle);
                            $ppnf.find('.wlm-message-pnf').html(passmsg);
                            $ppnf.removeAttr('style');
                            wlm.jq('#wlm-hpbtn-style').remove()
                            wlm.jq('#wlm-pwd-form').remove();
                        });
                    }
                }
            }
            ...
            if (wlm.currentISO) {
                funnavigateByLocation();
            } else {
                document.addEventListener('wlm_geoiso_allow', function (e) {
                    funnavigateByLocation();
                });
            }
        },
        ...
        getQueryParams: function () {
            ...
            return params != null && params != '' ? wlm.convertToArray(params) : {};
        },

        convertToArray: function (paramstr) {
            var arrPrms = {};
            var prm = paramstr.split("&");
            for (var i = 0; i < prm.length; i++) {
                var tmprm = prm[i].split("=");
                arrPrms[tmprm[0]] = tmprm[1];
            }
            return arrPrms;
        },

        addAfterElement: function (mainNode, childNode) {
            mainNode.parentNode.insertBefore(childNode, mainNode.nextSibling);
        },

        unlockByAuth: function (login) {
            if (!wlm.passId) return;
            var txtpass = wlm.jq(login).parent().closest('#wlm-pwd-form').find('#wlm-pwd');
            if (txtpass.length) {
                wlm.jq.ajax({
                    type: 'GET',
                    url: wlm.checkPasswordUrl,
                    async: true,
                    dataType: 'json',
                    data: { 'pw': txtpass.val(), 'f': '_authPass' },
                    success: function (response) {
                        var wlm_validate = undefined;   
                        for (let index = 0; index < wlm.navigateListByAuth.may.length; index++) {
                            if (wlm.navigateListByAuth.may[index].hp == response.key) {
                                wlm_validate = wlm.navigateListByAuth.may[index];
                            }
                            
                        }                        
                        // if (response.key && wlm.navigateListByAuth.may.find(i => i.hp == response.key) != undefined) {
                        if (response.key && wlm_validate != undefined) {
                            var h = response.key;
                            wlm.jq('#wlm-pwd-error').hide();
                            wlm.jq('#wlm-pwd').removeClass('wlm-error');
                            // wlm.setLocalStorage(h[1] + h[3] + h[2] + h[5] + wlm.navigateListByAuth.may.find(i => i.hp == response.key).lcid, h, { expires: 315360000 });
                            wlm.setLocalStorage(h[1] + h[3] + h[2] + h[5] + wlm_validate.lcid, h, { expires: 315360000 });
                            window.location.reload();
                        } else {
                            wlm.jq('#wlm-pwd-error').show();
                            wlm.jq('#wlm-pwd').addClass('wlm-error');
                        }
                    }
                });
            }
        },
        toggleContent: function (mode) {
            var ch, isdisplay = true,
                passNote = '',
                headerText = '';

            wlm.jq('#wlm-helper').remove();
            if (wlm.lockjs) return;
            var any_pass = false;
            if (wlm.navigateListByAuth.may.length) {
                wlm.jq.each(wlm.navigateListByAuth.may, function (i, item) {
                    ch = wlm.getLocalStorage(item.hp[1] + item.hp[3] + item.hp[2] + item.hp[5] + item.lcid);
                    if (!ch || ch != item.hp) {
                        isdisplay = false;
                        passNote = item.wlmPwdMsg;
                        //headerText = item.pp;
                        wlm.passId = item.lcid;
                        //break;                        
                    } else if (ch == item.hp) {
                        any_pass = true;
                        return false;
            ...
            if (isdisplay || any_pass) {
                wlm.jq(document).ready(function () {
                    wlm.jq('.wlm-content').removeAttr('style');
                    wlm.jq('#wlmpnf').remove();

                    //   if (!mode) {
                    //       wlm.preparePurchaseForm(); // by server
                    //   } else {
                    //       wlm.preparePurchaseFormByLocation(); // by client
                    //   }
                });

            } else {
                wlm.jq('#wlm-hpbtn-style').remove();
                wlm.jq('#wlm-pwd-form').removeAttr('style');
                wlm.jq('#wlm-pwd-msg').remove();
                wlm.jq('#wlm-wrapper').prepend('<div id="wlm-pwd-msg">' + passNote + '</div>');
                wlm.jq('#wlm-pwd').attr('placeholder', headerText);
                wlm.jq('.wlm-content').remove();
            }
        },
        initFormByAuth: function () {
            var ch, show = true;
            var passNote = '',
                headerText;

            if (!wlm.passId && wlm.hidePurchaseListByAuth.may.length) {
                wlm.jq.each(wlm.hidePurchaseListByAuth.may, function (i, item) {
                    ch = wlm.getLocalStorage(item.hp[1] + item.hp[3] + item.hp[2] + item.hp[5] + item.lcid);
                    if (!ch || ch != item.hp) {
                        show = false;
                        passNote = item.wlmPwdMsg;
                ...
                if (show) {
                    ...
                    wlm.jq(document).ready(function () {
                        var $forms = wlm.jq("form[action='/cart/add']");
                        if ($forms.length) {
                            wlm.jq('#wlm-pwd-form').removeAttr('style');
                            ...
                            $forms.each(function () {
                                ...
                                $btn.before(wlm.jq('#wlm-pwd-form'));
                                $btn.remove();
                                return;
                            });
                        }
        ...
        removeLinks: function () {
            if (wlm.arrHideLinks.length) {
                wlm.jq.each(wlm.arrHideLinks, function (i, v) {
                    wlm.jq(v).not('a[href*="cdn.shopify.com"]').hide();
                    for (let index = 0; index < wlm.jq(v).length; index++) {
                        var domelement=wlm.jq(v)[index];                        
                    wlm.jq(domelement).parent().children().length == 1 ? wlm.jq(domelement).parent().hide() : '';   
                       
                   } 
                    // wlm.jq(v).parent().children().length == 1 ? wlm.jq(sel).parent().hide() : '';
                });
            }
            wlm.removeItems();
            wlm.removeLinksByAuth();
            wlm.jq('.wlm-auto-hl-styles').remove();
            wlm.jq('.wlm-custom-hl-styles').empty();

        },

        removeLinksByAuth: function () {
            if (Object.keys(wlm.arrHideLinksByAuth.may).length) {
                wlm.jq.each(wlm.arrHideLinksByAuth.may, function (cn, sel) {
                    if (!wlm.getLocalStorage(cn)) {
                        wlm.jq(sel).hide();
                        wlm.jq(sel).parent().children().length == 1 ? wlm.jq(sel).parent().hide() : '';
                    }
                });
            }
        },
        ...
        getCookie: function (name) {
            if (wlm.supportsLocalStorage() && localStorage[name] != undefined) {
        ...
        removeItems: function () {
            function _removeItems() {
                var $collectionsItemsList = wlm.jq('.wlm-collection-item');
                if ($collectionsItemsList.length) {
                    $collectionsItemsList.each(function (i, item) {
                        var $item = wlm.jq(item);
                        ...
                        if (auth && auth.length) {
                            auth = auth.split(',');
                            wlm.jq.each(auth, function (i, cn) {
                                if (!wlm.getCookie(cn)) $item.remove();
                            });

                        } else if (can && can.length) {
                            if (can.indexOf(wlm.currentISO) < 0) $item.remove();

                        } else if (cannot && cannot.length) {
                            if (cannot.indexOf(wlm.currentISO) >= 0) $item.remove();
                        }

                        $item.removeAttr('data-eld-loc-can').removeAttr('data-eld-loc-cannot').removeAttr('data-eld-loc-auth');
                    });
                }

                var $itemsList = wlm.jq('.wlm-item');
                if ($itemsList.length) {
        ...
        initPage: function () {
            var loginurl = new RegExp('\/account\/login');
            if (loginurl.test(window.location.href)) {
                var params = wlm.getQueryParams();

                if (typeof params['return_to'] == 'string') {
                    var redirectUrl = document.getElementsByName("checkout_url");

                    if (typeof redirectUrl[0] == 'object') {
                        redirectUrl[0].setAttribute('value', params['return_to']);

                    } else {
                        var txtemail = document.getElementsByName("customer[email]");

                        if (typeof txtemail[0] == 'object') {
                            redirectUrl = document.createElement("input");
                            redirectUrl.setAttribute('type', 'hidden');
                            redirectUrl.setAttribute('name', 'checkout_url');
                            redirectUrl.setAttribute('value', params['return_to']);
                            wlm.addAfterElement(txtemail[0], redirectUrl);
                        }
                    }
                }
    ...
    wlm.jq(document).ready(function () {
        //if( typeof InstantClick == 'object' ) InstantClick.on('change', wlm.initPage);
        wlm.initPage();
    });
}